...
pipeline {
    ...
    stages {
        ...
        stage('Static Analysis') {
            parallel {
                ...
                stage('Detect Secrets') {
                    ...
                    post {
                        always {
                            archiveArtifacts artifacts: 'api/reports/detect-secrets-report.json, api/.secrets.baseline',
                                             fingerprint: true
                        }
                    }
                }
            ...
            steps {
                script {
                    dockerImagePython = docker.build(
                        "${DOCKER_IMAGEPYTHON}:${FINAL_IMAGE_TAG_PYTHON}",
        ...
        stage('Build Docker WAF') {
            ...
            steps {
                script {
                    dockerImageWaf = docker.build(
                        "${DOCKER_IMAGEWAF}:${FINAL_IMAGE_TAG_WAF}",
        ...
        stage('End-to-End Tests') {
            ...
            steps {
                sh """
                    ...
                    pytest api/web/tests_e2e \
                        ...
                        --disable-warnings \
                        -q \
                        --junitxml=e2e-results/e2e-report.xml \
            ...
            post {
                always {
                    junit testResults: 'e2e-results/*.xml',
                          allowEmptyResults: true
        ...
        stage('Deploy en Kubernetes') {
            steps {
                withCredentials([
                    usernamePassword(
                        credentialsId: 'ClaveSSHKubernetes',
